...
def create_context(session_id: str, tool_name: str, payload: str = "{}") -> ToolContext[ChatRunContext]:
    return ToolContext(
...
def test_run_mix_optimization_requires_planning(session_factory: Callable[[str], None]) -> None:
    ...
    result = asyncio.run(run_mix_optimization.on_invoke_tool(ctx, "{}"))
...
def test_run_mix_optimization_success(session_factory: Callable[[str], None]) -> None:
    ...
    asyncio.run(compute_planning_context.on_invoke_tool(planning_ctx, "{}"))

    opt_ctx = create_context(session_id, run_mix_optimization.name)
    result_json = asyncio.run(run_mix_optimization.on_invoke_tool(opt_ctx, "{}"))
