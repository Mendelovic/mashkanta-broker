...
def test_list_user_sessions_returns_summary(session_cleanup: Callable[[str], None]) -> None:
    user_id = "list-user"
...
def test_list_user_sessions_honors_limit(session_cleanup: Callable[[str], None]) -> None:
    user_id = "limit-user"
    first_id = _create_session(user_id, session_cleanup)
...
def test_get_session_detail_returns_history(session_cleanup: Callable[[str], None]) -> None:
    user_id = "detail-user"
    session_id, session = session_manager.get_or_create_session(None, user_id=user_id)
    session_cleanup(session_id)
...
def test_get_session_detail_rejects_other_user(session_cleanup: Callable[[str], None]) -> None:
    user_id = "owner-user"
