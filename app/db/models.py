...
class UserSession(Base):
    ...
    document_artifacts: Mapped[list["SessionDocumentArtifact"]] = relationship(
        back_populates="session", cascade="all, delete-orphan"
    )
...
class SessionDocumentArtifact(Base):
    """Stores sanitized metadata and extracts from uploaded documents."""
    ...
    original_filename: Mapped[str | None] = mapped_column(String(length=255))
    mime_type: Mapped[str | None] = mapped_column(String(length=128))
    document_type: Mapped[str] = mapped_column(String(length=32), default="unknown")
    ...
    session: Mapped[UserSession] = relationship(back_populates="document_artifacts")
